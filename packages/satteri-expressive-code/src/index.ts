...
import { getCodeBlockInfo } from "./utils.js";
...
export type SatteriExpressiveCodeOptions = Omit<ExpressiveCodeConfig, "themes"> & {
  ...
  getBlockLocale?:
    | (({
        ...
        document: SatteriExpressiveCodeDocument;
  ...
  customCreateBlock?:
    | (({
        ...
        input: ExpressiveCodeBlockOptions;
        document: SatteriExpressiveCodeDocument;
      }) => ExpressiveCodeBlock | Promise<ExpressiveCodeBlock>)
    | undefined;
  /**
  ...
  customCreateRenderer?:
    | ((
      ...
      ) => Promise<SatteriExpressiveCodeRenderer> | SatteriExpressiveCodeRenderer)
    | undefined;
};

export type ThemeObjectOrShikiThemeName =
  | BundledShikiTheme
  | ExpressiveCodeTheme
 ...
 * ```
 */
function expressiveCode(options: SatteriExpressiveCodeOptions = {}): () => HastPluginDefinition {
  const { tabWidth = 2, getBlockLocale, customCreateRenderer, customCreateBlock } = options;

  let asyncRenderer:
    | Promise<SatteriExpressiveCodeRenderer>
    | SatteriExpressiveCodeRenderer
    | undefined;
    ...
    return {
      ...
      element: {
        ...
        async visit(node, ctx) {
          ...
          const codeBlock = customCreateBlock
            ? await customCreateBlock({ input, document })
            : new ExpressiveCodeBlock(input);
          ...
          if (extraElements.length) {
            const firstChild =
              renderedGroupAst.children.length > 0 ? renderedGroupAst.children[0] : undefined;
